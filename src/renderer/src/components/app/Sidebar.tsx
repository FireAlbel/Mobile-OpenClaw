...
import { filterSidebarForRpaCutover } from '@renderer/services/rpa/RpaCutoverGate'
...
const MainMenus: FC = () => {
  ...
  const iconMap = {
    ...
    rpa_roles: <LayoutGrid size={18} className="icon" />,
    rpa_templates: <Workflow size={18} className="icon" />
  }
  ...
  const pathMap = {
    ...
    files: '/files',
    rpa_roles: '/rpa-roles',
    rpa_templates: '/rpa-workflows'
  }

  return filterSidebarForRpaCutover(sidebarIcons.visible)
    .filter((icon) => Object.hasOwn(iconMap, icon) && Object.hasOwn(pathMap, icon))
    .map((icon) => {
