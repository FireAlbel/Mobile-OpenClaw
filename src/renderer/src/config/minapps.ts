...
const loadCustomMiniApp = async (): Promise<MinAppType[]> => {
  try {
    ...
    if (!window.api || !window.api.file) {
      logger.warn('API not available, skipping custom mini apps loading')
      return []
    }
    ...
    try {
      ...
      try {
        ...
        logger.warn('Failed to create custom-minapps.json:', writeError as Error)
        return []
      }
    }
...
let allMinApps: MinAppType[] = [...ORIGIN_DEFAULT_MIN_APPS]

// 异步加载自定义小应用
const loadAllMinApps = async (): Promise<MinAppType[]> => {
  const customApps = await loadCustomMiniApp()
  allMinApps = [...ORIGIN_DEFAULT_MIN_APPS, ...customApps]
  return allMinApps
}
